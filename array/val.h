...
template<class T, value_kind SD, T V> std::ostream & operator<<(std::ostream & s, const value_t<T, SD, V> & val) {
	if(SD == sta) s << '$';
	return s << val.value;
}

namespace test_value {
	static t_dyn u2(HERE, VAR<int>(5).value == 5);
	static t_dyn u3(HERE, []() { VAR<char> v; v = 'a'; return v == 'a'; });
}
...
template<class A, class B, bool v = is_type<A>() && is_type<B>()> struct same_TYPE : false_t {};
template<class A, class B> struct same_TYPE<A, B, true> : same_t<typename A::type, typename B::type> {};
template<class A, class B> constexpr bool same_type() { return same_TYPE<A, B>::value; }
namespace test_same_type {
	static_assert(same_type<TYPE<int>, CONST<int, 3>>(), "same type");
	static_assert(!same_type<int, TYPE<bool>>(), "same type");
}
...
namespace test_same_value {
	...
	static_assert(!same_value<one, TYPE<int>>(), "same value");
